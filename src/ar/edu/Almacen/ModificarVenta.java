...
public class ModificarVenta extends JPanel
{
	...
	public ModificarVenta(JFrame ventanaUnica, Venta venta) 
	{
		
		costo=venta.getIngreso();
		...
		java.util.Date date = null;
		 ...
		 try 
		 {
			date = new SimpleDateFormat("yyyy-MM-dd").parse(venta.getFecha());
		 } 
		...
		traerProductos(modelo_de_tabla_productos,venta, comboBox);
		mostrarProductos(comboBox, venta);
		
		btnModificar.addActionListener(new ActionListener() 
		{
			public void actionPerformed(ActionEvent e) 
			{
				venta.setFecha(LocalDateTime.ofInstant(calen.getDate().toInstant(),
				...
				try 
				{
					...
					CallableStatement llamadaDeProcedimiento1 = conexion.prepareCall("{call [modificarVenta] (?,?,?)}"); // llamo procedimiento
					llamadaDeProcedimiento1.setInt(1, venta.getId_venta());
					llamadaDeProcedimiento1.setString(2, venta.getFecha());
					...
					{
					CallableStatement llamadaDeProcedimiento2 = conexion.prepareCall("{call [ingresarProductosXVenta] (?,?,?,?) }"); // llamo procedimiento
					...
					llamadaDeProcedimiento2.setFloat(4,p.getPrecio_venta());
	...
	private void traerProductos(DefaultTableModel modelo_tabla_productos, Venta venta, JComboBox comboBox)
	{
		...
		CallableStatement llamadaDeProcedimiento2 = conexion.prepareCall("{call [mostrarProductosXVentaConId] (?)}"); // llamo procedimiento
			...
			while(resultadosDeProcedimiento2.next())
			{
				...
				if(resultadosDeProcedimiento2.getString(6).contains("g"))
				{
					...
					datos.add(ppp.getPrecio_venta()*(resultadosDeProcedimiento2.getInt(9)/100));
				...
				if(resultadosDeProcedimiento2.getString(6).contains("un"))
				{
					...
					datos.add(resultadosDeProcedimiento2.getInt(9));
					datos.add(ppu.getPrecio_venta()*resultadosDeProcedimiento2.getInt(9));
	...
	public void mostrarProductos(JComboBox comboProductos,Venta venta)
	{
