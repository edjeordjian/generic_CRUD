...
public class AgregarCompra extends JPanel 
{
	...
	public AgregarCompra(JFrame ventanaUnica)
	{
		setBackground(new Color(128, 128, 128));
		ventanaUnica.setBounds(0, 0, 800,500);
		setLayout(null);
			...
			public void actionPerformed(ActionEvent e) 
			{
				for(Producto p:productosTempo)
				{
					if(p.getNombre()==comboProductos.getSelectedItem())
					{
						...
						if(salioBien)
						{
						for(Producto p2:productosTempo)
						{

							if(p2.getProveedor()!=id_proveedor_pd)
								comboProductos.removeItem(p2.getNombre());
		...
		btnFinalizar.addActionListener(new ActionListener() 
		{
			public void actionPerformed(ActionEvent e)
			{
				...
				try
				{
					...
					CallableStatement llamadaDeProcedimiento = conexion.prepareCall("{call [ingresarCompra] (?,?)}");				
					llamadaDeProcedimiento.setString(1, fecha);
					llamadaDeProcedimiento.setFloat(2, costo);
					llamadaDeProcedimiento.execute();


					CallableStatement llamadaDeProcedimiento2 = conexion.prepareCall("{call [ultimaComrpaId]}");				
					llamadaDeProcedimiento2.execute();
					ResultSet resultados = llamadaDeProcedimiento2.getResultSet();

					resultados.next();
					compra=new Compra(resultados.getInt(1),resultados.getString(2),new ArrayList<>(),resultados.getFloat(3));
					conexion.close();
				}
				...
				for(Producto p:productosSelec)
				{
					...
					try 
					{
						...
						CallableStatement llamadaDeProcedimiento1 = conexion2.prepareCall("{call [ingresarProductosXCompra] (?,?,?,?)}"); // llamo procedimiento
						...
						llamadaDeProcedimiento1.setFloat(4,p.getPrecio_compra());
						llamadaDeProcedimiento1.execute();// ejecutar procedimiento almacenado	
				 
						
							conexion2.close();
						compra.getProductos().add(p);
					} 
					catch (ClassNotFoundException e1)
					{
						e1.printStackTrace();
